...
const Teams = () => {
  ...
  const debouncedFetchPlayers = useCallback(
    debounce(async (searchTerm) => {
      ...
      try {
        ...
        setAvailablePlayers([...uniquePlayers.values()]);
      } catch (error) {
  ...
  useEffect(() => {
    if (searchTerm) {
      ...
      setAvailablePlayers([]); // Clears the players list if the search term is empty
    }
      ...
      setTeams((prevTeams) =>
        prevTeams.map((team) =>
          team.id === teamId
            ? {
                ...team,
                players: [...team.players, playersMap.get(playerId)],
              }
            : team
        )
      );
